#!/usr/bin/env python3
"""
scripts/run_aesplan_eval.py
============================
AesPlan quick evaluation: compare 4 variants on 20 prompts × 2 seeds.

Variants:
  1. uniform_dp     — DP with uniform L1 cost, combined-eps cache (DPCache baseline)
  2. aes_cost_only  — DP with AesMask-weighted cost, combined-eps cache
  3. diff_cfg_only  — DP with uniform cost, differential CFG at skip steps
  4. aesplan_full   — DP with AesMask-weighted cost + differential CFG (full method)
  5. accelaes_ref   — AccelAes fskip2 (existing method, for reference)

Metrics: ImageReward, HPSv2, Aesthetic Score, CLIP Score, time/img.
Outputs: outputs/aesplan_eval/summary.json

Usage:
  python scripts/run_aesplan_eval.py [--prompts 20] [--seeds 2] [--budget 10]
"""
...
# ---------------------------------------------------------------------------
# Metric helpers (inline evaluators, same as run_p0_eval.py)
# ---------------------------------------------------------------------------
...
def load_metrics(device: str = "cuda"):
    return {
        "ir": _IRMetric(device),
...
def score_image(metrics: dict, image, prompt: str) -> dict:
    return {
        "ir": metrics["ir"].score(image, prompt),
        "hps": metrics["hps"].score(image, prompt),
        "aesthetic": metrics["aesthetic"].score(image),
        "clip": metrics["clip"].score(image, prompt),
    }
...
def main():
    ...
    parser.add_argument("--budget", type=int, default=10,
                        help="Key step budget K (out of 30). Default=10 → 3× target")
    ...
    parser.add_argument("--output", type=str, default="outputs/aesplan_eval")
    parser.add_argument("--dtype", type=str, default="bf16")
    parser.add_argument("--load_calib", type=str, default=None,
                        help="Load existing calibration JSON (skip re-calibration)")
    parser.add_argument("--skip_accelaes", action="store_true",
                        help="Skip AccelAes reference run (faster)")
    ...
    print(f"\n{'='*60}")
    print(f"AesPlan Evaluation")
    print(f"  Budget: {args.budget}/{args.steps} key steps (~{args.steps/args.budget:.1f}× speedup)")
    print(f"  Prompts: {args.prompts}  Seeds: {args.seeds}  Total: {args.prompts * args.seeds} images/variant")
    print(f"{'='*60}\n")
    ...
    # --- Define variants ---
    variants = {
        "uniform_dp": dict(use_aes_cost=False, use_diff_cfg=False,
                           key_steps_override=uniform_key_steps,
                           desc=f"Uniform DP (L1 cost, uniform CFG), K={budget}"),
        "aes_cost_only": dict(use_aes_cost=True, use_diff_cfg=False,
                              desc=f"AesMask cost only (no diff-CFG), K={budget}"),
        "diff_cfg_only": dict(use_aes_cost=False, use_diff_cfg=True,
                              key_steps_override=uniform_key_steps,
                              desc=f"Diff-CFG only (uniform cost), K={budget}"),
        "aesplan_full": dict(use_aes_cost=True, use_diff_cfg=True,
    ...
    for var_name, var_kwargs in variants.items():
        ...
        for pi, prompt in enumerate(eval_prompts):
            for seed in seeds_list:
                ...
                print(f"  [{pi*args.seeds+seed+1:3d}/{args.prompts*args.seeds}] "
                      f"IR={s['ir']:.3f} HPS={s['hps']:.4f} Aes={s['aesthetic']:.3f} "
                      f"t={elapsed:.1f}s")
    ...
    # --- AccelAes reference (fskip2) ---
    if not args.skip_accelaes:
        print(f"\n{'─'*50}")
        print("Reference: AccelAes fskip2")
        print(f"{'─'*50}")
        ...
        for pi, prompt in enumerate(eval_prompts):
            for seed in seeds_list:
                ...
                img = wrapper.generate_accelerated_dual(
                    ...
                    skip_ratio=0.50, s_fg=args.s_fg, s_bg=args.s_bg,
                    mask_step=args.mask_step, full_skip_interval=2,
        ...
        results["accelaes_ref"] = {
            "desc": "AccelAes fskip2 (2.11× reference)",
            "mean_time": float(np.mean(times)),
            "mean_ir": float(np.mean([s["ir"] for s in scores])),
            "mean_hps": float(np.mean([s["hps"] for s in scores])),
            "mean_aesthetic": float(np.mean([s["aesthetic"] for s in scores])),
    ...
    for var_name, r in results.items():
        if baseline_time is None:
            # Estimate baseline from AccelAes ref or dense (12.37s)
            baseline_time = 12.37
